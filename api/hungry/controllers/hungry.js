...
module.exports = {
    ...
    async create(ctx) {
        ...
        if (ctx.is('multipart')) {
            ...
            data.user = [id];
            entity = await strapi.services.hungry.create(data, { files });
        } else {
            ctx.request.body.user = [id];
            entity = await strapi.services.hungry.create(ctx.request.body);
        }
        ...
        //Preparing Command
        const fileName = "hungry_" + uuidv4().substring(0, 8) + ".txt";
        const host = "http://" + process.env.HOST + ":" + process.env.PORT;
        const cmd = `~/scripts/hungry/${scriptFile} ` +
            ' -a "' + entity.appName + '"' +
            ' -b "' + entity.baseUrl + '"' +
            (entity.phone ? ' -c "' + entity.phone + '"' : '') +
            ' -o "' + entity.onesignal + '"' +
        ...
        return sanitizeEntity(entity, { model: strapi.models.hungry });
    },
};