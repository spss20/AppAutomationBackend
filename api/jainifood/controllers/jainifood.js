...
module.exports = {
    ...
    async create(ctx) {
        ...
        if (ctx.is('multipart')) {
            ...
            data.user = [id];
            entity = await strapi.services.jainifood.create(data, { files });
        } else {
            ctx.request.body.user = [id];
            entity = await strapi.services.jainifood.create(ctx.request.body);
        }
        ...
        //Preparing Command
        const fileName = "jainifood_" + uuidv4().substring(0, 8) + ".txt";
        const host = "http://" + process.env.HOST + ":" + process.env.PORT;
        const cmd = `~/scripts/jainifood/${scriptFile} ` +
            ' -a "' + entity.appName + '"' +
            ' -b "' + entity.baseUrl + '"' +
        ...
        return sanitizeEntity(entity, { model: strapi.models.jainifood });
    },
};